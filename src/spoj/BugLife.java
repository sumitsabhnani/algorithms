...
public class BugLife {
    ...
    public static void main(String args[]) {
        ...
        for (int j = 0; j < testCases; j++) {
            ...
            for (int i = 0; i < bugs; i++) {
                isBipartite = isBipartite(bugGraph, colors, i);
                if (!isBipartite)
                    break;
            }
    ...
    static boolean isSafe(AdjacencyListGraph graph, int v, int[] colors, int c) {
    ...
    static boolean isBipartite(AdjacencyListGraph graph, int[] colors, int src) {
        for (int c = 1; c < 3; c++) {
            colors[src] = c;
            if (isSafe(graph, src, colors, c))
                return true;
            colors[src] = 0;
        }
        return false;
